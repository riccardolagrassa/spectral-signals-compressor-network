...
class Encoder(nn.Module):
    def __init__(self):
        ...
        self.mu = nn.Linear(256*2, opt.latent_dim)
        self.logvar = nn.Linear(256*2, opt.latent_dim)
    ...
    def forward(self, img):
        ...
        x_visible_flat = x_visible.view(img.shape[0], -1)
        x_ext_vnir_flat = x_ext_vnir.view(img.shape[0], -1)

        x_merge = torch.cat([x_visible_flat, x_ext_vnir_flat], 1)

        mu = self.mu(x_merge)
        logvar = self.logvar(x_merge)
...
class Decoder(nn.Module):
    def __init__(self):
        ...
        self.visible_spectrum_decoder = nn.Sequential(
            ...
            nn.BatchNorm1d(512),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Linear(512, int(np.prod(img_v_shape))),
            nn.Sigmoid()
        )

    def forward(self, z):
        v_img_flat = self.visible_spectrum_decoder(z)
        ext_img_flat = self.ext_vnir_spectrum_decoder(z)
        v_img = v_img_flat.view(v_img_flat.shape[0], * img_v_shape)
        ext_img = ext_img_flat.view(ext_img_flat.shape[0], * img_ext_shape)
        x_merge = torch.cat([v_img, ext_img], 1)
        return x_merge
...
results_dir = '/home/super/rlagrassa/inaf_compression_project/experiments/aae_twoB/results_slice_'+str(dataset_slice)
