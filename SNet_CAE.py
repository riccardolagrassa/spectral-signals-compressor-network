...
def validate():
    ...
    with torch.no_grad():
        for index, (data, filename_packed) in enumerate(valid_dataloader):
            data = data.cuda()
            #selection_bands, index_bands = snet(data)
            encoded, a, sorted_bands = encoder(data)
            decoded = decoder(encoded)
            for j, filename, real_data, idx_bands in zip(decoded, filename_packed,a, sorted_bands):
                ds=Ds_build.main(valid_dir+filename)
                if index == 0:
                    print("Selected bands: ", idx_bands.cpu().numpy())
            ...
            pq_psnr = piq.psnr(decoded, a, data_range=1.)
            pq_ssim = piq.ssim(decoded, a, data_range=1.)

            total_psnr_value += pq_psnr
            total_ssim += pq_ssim

            for b in range(9):
                pq_psnr = piq.psnr(decoded[:, None, b, :, :], a[:, None,b, :, :], data_range=1.)
                pq_ssim=piq.ssim(decoded[:, None,b, :, :], a[:, None,b, :, :], data_range=1.)
                pq_fsim=piq.fsim(decoded[:, None,b, :, :], a[:, None,b, :, :], data_range=1., chromatic=False)
...
#snet= SNet_model.SNet(opt.channels)
encoder = SNet_model.Encoder(opt.latent_dim)
decoder = SNet_model.Decoder(opt.latent_dim)
criterion.cuda()
...
results_dir = '/home/super/rlagrassa/inaf_compression_project/experiments/SNet_CAE_100k/results_slice_'+str(dataset_slice)+'_'+str(opt.latent_dim)
...
#snet = nn.DataParallel(snet).cuda()
...
if opt.resume:
    ...
    state_dict_snet = torch.load(opt.model_path_selector)
    snet.load_state_dict(state_dict_snet)
...
def train(epoch):
    for i, (imgs, filename) in enumerate(train_dataloader):
        ...
        #selection_bands, _ = snet(real_imgs)
        encoded, a, _  = encoder(real_imgs)
        decoded  = decoder(encoded)

        g_loss =  criterion(decoded,a)
...
def save_model(avg_psnr_value, avg_ssim_value, epoch, avg_score_values):
    if avg_ssim_value > avg_score_values:
        print("Epoch: ", epoch," SSIM: ", avg_ssim_value, " PSNR: ",
              avg_psnr_value)
        torch.save(encoder.state_dict(), os.path.join(opt.save_path_model, "SNet_CAE_encoder_best.pth"))
        torch.save(decoder.state_dict(), os.path.join(opt.save_path_model, "SNet_CAE_decoder_best.pth"))
        #torch.save(snet.state_dict(), os.path.join(opt.save_path_model, "SNet_CAE_selectorNet_best.pth"))
