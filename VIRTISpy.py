...
class VIRTISpy:
	...
	def reduceHeader(self,header):
		...
		for i in range(len(header)):
			if len(header[i])>2:
				# if (header[i][2][0] == b'{' and header[i][-1][-1] !='}'):
				# 	header[i].extend(header[i+1])
				# 	del header[i+1]
				# 	fl=1
				# 	break

				numbers=re.findall(r"([\d.]*\d+)", str(header[i][2:]))
				if numbers:
					header[i][2:]=numbers

				else:
					apex_name = re.findall(r"\"([^:]+)\"", str(header[i][2:]))
					if apex_name:
						apex_name_cleaned=str(apex_name).replace('b','').replace('\"','').replace('\'','').replace('\\','').replace(',','').replace('[','').replace(']','')
						header[i][2]=apex_name_cleaned
				# if (header[i][2][0] == b'(' and header[i][-1][-1] !=')'):
				# 	header[i].extend(header[i+1])
				#del header[i+1]
				#fl=1
	...
	def readHeader(self,fn):
		...
		while 1:
			...
			if len(line) == 1 and line[0] == b'END': # End of header
				break
	...
	def getBand(self,band):
		if band > self._dim[0]:
			...
			if self._dataType == b'VIRTIS DATA':
				#ret= transpose(self._qube[band,:,:])
				ret = self._qube
			elif self._dataType== b'VIRTIS GEOMETRY':
	...
	def readPDSFile(self,fn):
		...
		self._Channel=self.headerValue(b'ROSETTA:CHANNEL_ID')
		recByte=int(self.headerValue(b'RECORD_BYTES'))
		qubPos=int(self.headerValue(b'^QUBE'))
		dataPos=qubPos -1
		fl=open(fn,'rb')
		fl.seek(dataPos*recByte)
		core=self.headerValue(b'CORE_ITEMS')
		crType=self.headerValue(b'CORE_ITEM_TYPE')
		sfxItem=self.headerValue(b'SUFFIX_ITEMS')
		self._dim=[int(core[0]),int(core[1]),int(core[2])]
		if crType == b'"REAL"' or crType ==b'REAL':
			tp='f'
			crByte=4
		elif crType == b'MSB_INTEGER':
			crByte=int(self.headerValue(b'CORE_ITEM_BYTES'))
			if crByte ==4:
				tp='i'
			elif crByte == 2:
				tp='h'
		...
		self._dataType=self.headerValue(b'STANDARD_DATA_PRODUCT_ID')[0]
		if self._dataType == 'VIRTIS DATA':
			procLev=int(self.headerValue(b'PROCESSING_LEVEL_ID'))
			if procLev == 3:
				scetDim=2
